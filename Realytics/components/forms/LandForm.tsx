    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-gray-700">Zoning Type <span className="text-red-400">*</span></label>
            <select
        ...
        <div className="mt-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {(Object.keys(form.utilities) as (keyof LandInputs['utilities'])[]).map((util) => (
              ...
              >
                <span className={`w-2 h-2 rounded-full ${form.utilities[util] ? 'bg-green-400' : 'bg-gray-400'}`} />
      ...
      {form.purchasePrice > 0 && form.expectedAppreciationRate > 0 && (
        <div className="p-4 bg-green-500/5 border border-green-500/20 rounded-xl">
          <p className="text-xs text-gray-500 mb-2 font-medium">Appreciation Preview</p>
          <div className="grid grid-cols-3 gap-4">
            <div>
              <div className="text-xs text-gray-500">Current Value</div>
              <div className="text-sm font-bold text-gray-900">${form.purchasePrice.toLocaleString()}</div>
            </div>
            <div>
