...
export default function ComparableProperties({
  ...
  const avgCapRate =
    comparables.filter((c) => (c.capRate ?? 0) > 0).reduce((sum, c) => sum + (c.capRate ?? 0), 0) /
    (comparables.filter((c) => (c.capRate ?? 0) > 0).length || 1);
  const avgPricePerSqft =
    comparables.reduce((sum, c) => sum + (c.pricePerSqft ?? 0), 0) / comparables.length;
      ...
      <div className="space-y-3">
        {comparables.map((comp, idx) => {
          const capRateComparison =
            subjectCapRate && (comp.capRate ?? 0) > 0
              ? (comp.capRate ?? 0) - subjectCapRate
              : null;

          return (
            <div
              key={comp.id}
              className="p-4 bg-gray-100 border border-gray-300/50 rounded-xl hover:border-gray-400 transition-colors"
            >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-medium text-gray-700">
                        {comp.size ? `${comp.size.toLocaleString()} sqft` : "—"}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500">$/sqft</div>
                      <div className="text-sm font-medium text-gray-700">
                        {comp.pricePerSqft ? `$${comp.pricePerSqft.toLocaleString()}` : "—"}
                        {avgPricePerSqft > 0 && comp.pricePerSqft && (
                    ...
                    {(comp.capRate ?? 0) > 0 && (
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          <span className={clsx('text-sm font-bold', (comp.capRate ?? 0) >= 7 ? 'text-green-400' : (comp.capRate ?? 0) >= 5 ? 'text-yellow-400' : 'text-red-400')}>
                            {(comp.capRate ?? 0).toFixed(1)}%
                          </span>
